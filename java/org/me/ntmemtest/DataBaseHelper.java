...
// partial delete - clears all bibles
   public void deleteAllBiblesYesIAmSure() {
    ...
    /*public int upgradeTable(){
    	String ssql = "select * from verses where book = 'seven'";
    ...
    public boolean versesToCSV() throws IOException{
    	boolean successx = false;
    	String ssql = "select * from verses";
    	try{
    	//resetDbConnection();
    	Cursor c = myDataBase.rawQuery(ssql, null);
    	if (c != null){
    		...
    		if (c.moveToFirst()){
    			File myFile;
    			myFile = new File(Environment.getExternalStorageDirectory()+"/MyVerses.csv");
    	        myFile.createNewFile();
    	        FileOutputStream fOut = new FileOutputStream(myFile);
    	        OutputStreamWriter myOutWriter = new OutputStreamWriter(fOut);
    	        myOutWriter.append("Book,Chapter,Verse,Version,Plus,Minus,Weight");
    	        myOutWriter.append("\n");
    			do {
    				String book1 = c.getString(bookColumn);
                    int chapter1 = c.getInt(chapterColumn);
                    int verse1 = c.getInt(verseColumn);
                    String version1 = c.getString(versionColumn);
                    int plus1 = c.getInt(plusColumn);
                    int minus1 = c.getInt(minusColumn);
                    int verseWeight = calcWeightValue(plus1, minus1);
                    myOutWriter.append(book1+","+String.valueOf(chapter1)+","+String.valueOf(verse1)+","+version1+
                    		","+String.valueOf(plus1)+","+String.valueOf(minus1)+","+String.valueOf(verseWeight));
                    myOutWriter.append("\n");
    			} while (c.moveToNext());
        ...
        if (c != null){
            if (c.moveToFirst()){
                ...
                do {
                    ...
                    /*String[][] tempVar;
                    tempVar = new String[4][tempVerses[0].length + 1];
                    for (int Dimension0 = 0; Dimension0 < tempVerses.length; Dimension0++){
                        int CopyLength = Math.min(tempVerses[0].length, tempVar[0].length);
                        for (int Dimension1 = 0; Dimension1 < CopyLength; Dimension1++){
                            tempVar[Dimension0][Dimension1] = tempVerses[Dimension0][Dimension1];
                        }
                    }
                    tempVerses = tempVar;
                    tempVerses[0][tempVerses[0].length-1] = book1;
                    tempVerses[1][tempVerses[0].length-1] = Integer.toString(chapter1);
                    tempVerses[2][tempVerses[0].length-1] = Integer.toString(verse1);
                    tempVerses[3][tempVerses[0].length-1] = Integer.toString(verseWeight);*/
